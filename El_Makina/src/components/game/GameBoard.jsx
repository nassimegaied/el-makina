...
const GameBoard = () => {
  ...
  if (!gameState || gameState.status === 'waiting') {
    return (
      <div className="lobby-screen px-4">
        <h1 className="lobby-title text-4xl md:text-6xl">EL MAKINA</h1>
        <p className="lobby-subtitle text-sm md:text-xl">Le pouvoir s'achète</p>
        {!gameState ? (
          <div className="flex flex-col items-center w-full mt-8">
            <input type="text" placeholder="Ton Prénom" className="lobby-input w-full max-w-sm" value={playerName} onChange={(e) => setPlayerName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleJoin()}/>
            <button onClick={handleJoin} className="btn-join w-full max-w-sm mt-4">Rejoindre la table</button>
          </div>
        ) : (
          <div className="text-center mt-8 bg-gray-900/80 p-6 md:p-8 rounded-3xl border border-gray-800 shadow-2xl w-full max-w-md">
            <h2 className="text-lg md:text-xl mb-6 font-bold text-gray-400 uppercase tracking-widest">Joueurs à table</h2>
            ...
            {gameState.players.length >= 3 ? (
               <button onClick={handleStartGame} className="btn-start w-full">Lancer la partie</button>
            ) : (
               <p className="text-red-500 font-bold animate-pulse text-sm md:text-base">En attente de {3 - gameState.players.length} joueur(s)...</p>
            )}
          </div>
        )}
      </div>
  ...
  if (gameState.status === 'game_over') {
    ...
    return (
      <div className="lobby-screen flex flex-col items-center justify-center min-h-screen text-center p-4">
        <div className="bg-gray-900/90 p-8 md:p-12 rounded-3xl border-4 border-yellow-500 shadow-[0_0_100px_rgba(234,179,8,0.5)] max-w-3xl w-full">
            ...
            <h1 className="text-4xl md:text-8xl font-black text-yellow-500 drop-shadow-[0_0_30px_rgba(234,179,8,0.8)] mb-4 md:mb-6 leading-tight">
                {amIWinner ? "VICTOIRE !" : "FIN DE PARTIE"}
            </h1>
            <p className="text-xl md:text-3xl text-gray-300 font-bold tracking-widest mb-8 md:mb-12 uppercase">
                {amIWinner ? "Tu es le nouveau Parrain" : `${winner?.name} a pris le pouvoir`}
            </p>
  ...
  const handleInitiateRole = (role, requiresTarget) => {
      if (requiresTarget) {
          ...
          socket.emit('claim_role', { role: role, targetId: null });
      }
  };
  ...
  return (
    <div className="game-container flex flex-col h-screen overflow-hidden relative">
      ...
      {/* NOUVEAU BOUTON REGLES */}
      <button 
        onClick={() => setShowRules(true)} 
        className="absolute top-4 left-4 z-[65] bg-gray-900/80 text-yellow-500 border border-yellow-600/50 rounded-full w-10 h-10 md:w-12 md:h-12 flex items-center justify-center font-black text-xl hover:bg-gray-800 hover:scale-110 transition-all shadow-[0_0_15px_rgba(202,138,4,0.3)] backdrop-blur-md"
      >
        ?
      </button>

      {/* AFFICHE LES REGLES SI ON CLIQUE */}
      {showRules && <RulesModal onClose={() => setShowRules(false)} />}
      
      {/* Reste de ton code (ActionModal, etc.)... */}
      ...
      {isPendingPoliceDecision && (
          <div className="discard-overlay flex flex-col items-center !z-[80] p-4">
              <div className="bg-gray-900 border-l-4 border-r-4 border-cyan-500 p-6 md:p-8 rounded-xl text-center shadow-[0_0_80px_rgba(6,182,212,0.3)] max-w-lg w-full">
                  <h2 className="text-2xl md:text-3xl font-black text-cyan-400 mb-2 tracking-widest uppercase">Interrogatoire</h2>
                  {iAmPolice ? (
                      <>
                          <p className="text-sm md:text-base text-gray-300 mb-4 md:mb-6">Tu fouilles <span className="font-bold text-white">{pendingPoliceDecision.targetName}</span>. Voici sa carte :</p>
                          ...
                          <div className="flex flex-col md:flex-row gap-3 md:gap-4">
                              <button onClick={() => socket.emit('police_decision', 'keep')} className="w-full bg-cyan-700 text-white font-black py-3 md:py-4 rounded-xl shadow-[0_5px_0_rgb(22,78,99)] active:translate-y-[5px] active:shadow-none transition-all">GARDER</button>
                              <button onClick={() => socket.emit('police_decision', 'change')} className="w-full bg-red-700 text-white font-black py-3 md:py-4 rounded-xl shadow-[0_5px_0_rgb(153,27,27)] active:translate-y-[5px] active:shadow-none transition-all">CHANGER</button>
                          </div>
                      </>
                  ) : (
                      <p className="text-base md:text-xl text-gray-300 animate-pulse mt-4">{amIBeingInspected ? `Le policier examine ta carte en secret...` : `${pendingPoliceDecision.policeName} examine secrètement une carte de ${pendingPoliceDecision.targetName}...`}</p>
                  )}
              </div>
          </div>
      )}
      ...
      {guessTarget && (
          <div className="discard-overlay flex flex-col items-center !z-[80] p-4">
              <div className="bg-gray-900 border-l-4 border-r-4 border-emerald-500 p-6 md:p-8 rounded-xl text-center shadow-[0_0_80px_rgba(16,185,129,0.3)] max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                  <h2 className="text-2xl md:text-3xl font-black text-emerald-400 mb-2 tracking-widest uppercase">Roulette Russe</h2>
                  <p className="text-sm md:text-base text-gray-300 mb-4 md:mb-6">Quelle carte penses-tu que la cible cache ? <br/><span className="text-xs md:text-sm text-red-400 font-bold">(Erreur = Tu perds 4🪙)</span></p>
                  ...
                  <button onClick={() => setGuessTarget(null)} className="text-gray-500 hover:text-white font-bold uppercase underline text-sm md:text-base">Annuler la frappe</button>
              </div>
          </div>
      )}
      ...
      {iAmLoser && (
        <div className="discard-overlay p-4">
            <div className="discard-box w-full max-w-md p-6 md:p-10">
                <h2 className="text-2xl md:text-3xl font-black text-red-500 mb-2 md:mb-4 tracking-widest uppercase">{pendingDiscard?.wasLying ? "Coupable" : "Exécution"}</h2>
                <p className="text-base md:text-xl text-gray-300">{pendingDiscard?.wasLying ? "Tu t'es fait attraper !" : "Le meurtre est commandité !"}</p>
                <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t border-gray-800">
                    <p className="text-lg md:text-2xl font-black text-white animate-pulse">Choisis une carte à sacrifier.</p>
                </div>
            </div>
        </div>
      )}
      ...
      {iAmExchanger && (
          <div className="discard-overlay flex flex-col items-center !z-[70] p-4">
              <div className="bg-gray-900 border-l-4 border-r-4 border-blue-500 p-6 md:p-8 rounded-xl text-center shadow-[0_0_80px_rgba(59,130,246,0.3)] w-full max-w-3xl mb-4 md:mb-8 max-h-[90vh] overflow-y-auto">
                  <h2 className="text-2xl md:text-3xl font-black text-blue-400 mb-2 tracking-widest uppercase">Secret d'État</h2>
                  <p className="text-sm md:text-base text-gray-300 mb-4 md:mb-6">Choisis <span className="font-bold text-white">{me.cards.length}</span> carte(s) à garder en main.</p>
      ...
      {/* TOP RING (SCROLLABLE ON MOBILE) */}
      <div className="top-ring flex overflow-x-auto snap-x px-4 py-6 gap-4 md:gap-12 md:justify-center w-full">
        {opponents.map(opp => {
          ...
          return (
            <div key={opp.id} onClick={() => isTargetable && handleOpponentClick(opp.id)} className={`avatar-wrapper snap-center shrink-0 flex flex-col items-center ${!opp.isAlive ? 'opacity-30 grayscale' : ''} ${isTargetable ? 'cursor-pointer hover:-translate-y-2' : ''}`}>
              ...
              {opp.isAlive ? (
                  <div className="flex flex-col items-center mt-1">
                      <div className="bg-gray-900/80 px-2 md:px-3 py-1 rounded-full border border-gray-700 text-[10px] md:text-xs whitespace-nowrap">
                        <span className="text-yellow-400 font-bold">{opp.coins} فلوس</span> <span className="text-gray-600 mx-1">|</span> <span className="text-blue-300 font-bold">{opp.cards} كارطة</span>
                      </div>
                      {/* NOUVEAU : LE GAME LOG EN DESSOUS */}
              ...
              ) : (<span className="text-red-600 font-black text-[9px] md:text-[10px] mt-1 uppercase tracking-widest bg-black/50 px-2 py-0.5 rounded">Éliminé</span>)}
            </div>
          );
        })}
      </div>
      ...
      <div className="middle-table flex-grow flex items-center justify-center">
        <div className="bg-gray-900/60 p-4 md:p-8 rounded-full border border-gray-800 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col items-center justify-center w-32 h-32 md:w-48 md:h-48">
          <p className="text-sm md:text-2xl font-bold text-gray-500 tracking-[0.3em] uppercase mb-0 md:mb-1">البانكة</p>
          <p className="text-4xl md:text-6xl font-black text-yellow-500 drop-shadow-md">{gameState.bankCoins}</p>
        </div>
      </div>

      {/* BOTTOM DASHBOARD (ADAPTIVE FOR MOBILE) */}
      <div className="bottom-dashboard bg-gray-950/95 backdrop-blur-xl border-t-2 border-gray-800 w-full z-[60] shadow-[0_-15px_40px_rgba(0,0,0,0.8)] px-2 md:px-6 py-4 md:pb-8 flex flex-col h-auto min-h-[35%] justify-end">
        {me?.isAlive === false ? (
            <div className="flex-grow flex items-center justify-center">
                <div className="text-center bg-gray-900/80 border border-gray-800 p-6 md:p-8 rounded-3xl"><p className="text-red-600 font-black text-3xl md:text-5xl tracking-widest drop-shadow-lg mb-1 md:mb-2">خسرت</p><p className="text-gray-500 font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase text-xs md:text-base">Spectateur</p></div>
            </div>
        ) : (
            <>
                <div className="flex flex-row justify-between items-end mb-4 md:mb-8">
                  <div className="flex flex-col items-start bg-gray-900/80 p-2 md:p-4 rounded-xl border border-gray-800 min-w-[70px] md:min-w-[100px]">
                    <p className="text-[8px] md:text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-0 md:mb-1">Ma Caisse</p>
                    <p className="text-2xl md:text-4xl font-black text-yellow-500 leading-none">{me?.coins}</p>
                    {/* Game Log de ton joueur */}
                    <p className="text-[8px] text-yellow-600/70 italic mt-1 max-w-[80px] truncate">{me?.lastAction}</p>
                  </div>
                  
                  {isPendingDiscard && !iAmLoser ? (
                      <div className="flex flex-col items-end gap-2 animate-pulse"><span className="text-red-500 font-black tracking-widest uppercase bg-red-950/30 px-2 md:px-4 py-1 md:py-2 rounded-lg border border-red-900/50 text-[10px] md:text-sm text-right">☠️ Exécution de {pendingDiscard?.playerName}...</span></div>
                  ) : isPendingExchange && !iAmExchanger ? (
                      <div className="flex flex-col items-end gap-2 animate-pulse"><span className="text-blue-500 font-black tracking-widest uppercase bg-blue-950/30 px-2 md:px-4 py-1 md:py-2 rounded-lg border border-blue-900/50 text-[10px] md:text-sm text-right">🔄 {pendingExchange?.playerName} échange ses cartes...</span></div>
                  ) : isTargeting ? (
                      <div className="flex flex-col items-end gap-2"><span className="text-red-500 font-black tracking-widest uppercase bg-red-950/50 px-3 md:px-4 py-1.5 md:py-2 rounded-lg border border-red-900 animate-pulse text-xs md:text-sm">Choisis ta cible !</span><button onClick={() => setIsTargeting(false)} className="text-gray-400 hover:text-white uppercase font-bold text-[10px] md:text-xs">Annuler</button></div>
                  ) : (
                      <div className="flex flex-col items-end gap-2 w-full ml-2">
                        {isMyTurn && !isInterfaceBlocked && (
                            <span className="text-green-500 font-black tracking-widest uppercase bg-green-950/30 px-3 md:px-4 py-1 rounded-md border border-green-900/50 text-[10px] md:text-sm mb-1">C'est ton tour</span>
                        )}
                        <div className="flex gap-1.5 md:gap-3 flex-wrap justify-end overflow-y-auto max-h-[120px] md:max-h-none pr-1">
                          {/* CSS inline pour rendre les boutons plus petits sur mobile sans casser l'index.css */}
                          ...
                          <button onClick={handleSalary} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-salary act-btn">شهريّة (+1)</button>
                          <button onClick={handleAid} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-aid act-btn">إعانة (+2)</button>
                          <button onClick={() => handleInitiateRole('percepteur', false)} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-tax act-btn">ضريبة</button>
                          <button onClick={() => handleInitiateRole('police_inspect', true)} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-police act-btn">بوليس</button>
                          <button onClick={() => handleInitiateRole('colonel_guess', true)} disabled={!isMyTurn || isInterfaceBlocked || (me?.coins < 4)} className="action-btn-base btn-colonel act-btn">كولونيل (-4)</button>
                          <button onClick={() => handleInitiateRole('politicien', false)} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-politicien act-btn">سياسي</button>
                          <button onClick={() => handleInitiateRole('homme_daffaires', false)} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-role act-btn">رجل أعمال (+4)</button>
                          <button onClick={() => handleInitiateRole('voleur', true)} disabled={!isMyTurn || isInterfaceBlocked} className="action-btn-base btn-steal act-btn">سارق (+2)</button>
                          <button onClick={() => handleInitiateRole('terroriste', true)} disabled={!isMyTurn || isInterfaceBlocked || (me?.coins < 3)} className="action-btn-base btn-kill act-btn">إرهابي (-3)</button>
                          <button onClick={() => handleInitiateRole('coup_detat', true)} disabled={!isMyTurn || isInterfaceBlocked || (me?.coins < 7)} className={`action-btn-base act-btn border border-transparent ${me?.coins >= 7 ? 'bg-red-800 text-white shadow-lg' : 'bg-gray-800 text-gray-500 opacity-50'}`}>إنقلاب (-7)</button>
                        </div>
                      </div>
                  )}
                </div>
